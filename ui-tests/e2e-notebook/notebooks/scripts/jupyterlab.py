...
async def add_cell(
    ...
    """
    # Get all cells in the active panel and select the last one
    panel = await get_active_panel(page, timeout)
    cells = panel.locator('.jp-Cell')
    ...
    insert_button = panel.locator('.jp-Toolbar-item[data-jp-item-name="insert"]')
...
async def move_cell_up(
    page: Page,
    index: int,
    timeout: int = 30000
):
    """Move a cell up by one position using Edit menu.

    Args:
        page: Playwright page object
        index: Cell index to move (0-based)
        timeout: Timeout in milliseconds
    """
    # Select the cell to move
    await select_cell(page, index, timeout)

    # Click Edit menu
    edit_menu = page.locator('.lm-MenuBar-itemLabel').filter(has_text=re.compile(r'^Edit$'))
    await expect(edit_menu).to_be_visible(timeout=timeout)
    ...
    # Click "Move Cell Up" menu item
    move_up_item = dropdown_menu.locator('.lm-Menu-item').filter(has_text=re.compile(r'Move Cell Up'))
    await expect(move_up_item).to_be_visible(timeout=timeout)
    await move_up_item.click()
...
async def copy_cell(
    ...
    """Copy a cell using Edit menu.

    Args:
        page: Playwright page object
        index: Cell index to copy (0-based)
        timeout: Timeout in milliseconds
    """
    ...
    # Click "Copy Cell" menu item
    copy_item = dropdown_menu.locator('.lm-Menu-item').filter(has_text=re.compile(r'^Copy Cell'))
    await expect(copy_item).to_be_visible(timeout=timeout)
...
async def paste_cell_below(
    page: Page,
    timeout: int = 30000
):
    """Paste a cell below the currently selected cell using Edit menu.
    ...
    # Click "Paste Cell Below" menu item
    paste_item = dropdown_menu.locator('.lm-Menu-item').filter(has_text=re.compile(r'Paste Cell Below'))
    await expect(paste_item).to_be_visible(timeout=timeout)
    await paste_item.click()
